...
"""
mAIpper v0.10 - Pentest Tool Analysis & Obsidian Export Tool
...
def build_misc_ollama_prompt(
    ...
    # Truncate content for prompt if very large
    max_content = 100_000
...
""".strip()
    ...
    logging.debug(f"Built Misc Ollama prompt ({len(prompt)} chars)")
    return prompt
...
def _write_loot_credentials_page(
    ...
    for host_key, loot_files in sorted(loot_data["host_loot"].items()):
        ...
        for c in all_creds[:50]:
            ...
            lines.append(f"| `{c['username']}` | `{pw}` | {c['cred_type']} | {c['source']} |")
        lines.append("")
    ...
    if analysis_by_host:
        for host_key, analysis in analysis_by_host.items():
            if host_key == "_campaign":
                overview_lines += ["## Campaign-Level Loot Analysis", ""]
            else:
                overview_lines += [f"## Analysis — {host_key}", ""]
            if model_name:
                overview_lines.append(f"Model: `{model_name}`\n")
            overview_lines.append(analysis)
            overview_lines.append("")
...
def _watch_loop(args, base: Path) -> None:
    ...
    print("      /refresh    — force re-process all files")
    ...
    if current:
        logging.info("[Watch] Initial processing...")
        try:
    ...
    try:
        while True:
            # Check for file changes before prompting
            current = _snapshot_scan_files(base, args)
            ...
            # Interactive prompt
            try:
                print("\033[90m  Enter to check for changes · type a question · /help for commands\033[0m")
                user_input = input("mAIpper> ").strip()
            except EOFError:
                break

            if not user_input:
                # Check for pending deep dives (checked investigation boxes)
                if not args.no_ollama:
                    ...
                    print("  No changes detected (LLM disabled).")
                continue
            ...
            if user_input.lower() in ("/quit", "/exit", "/q"):
                ...
                print("  Refreshing (reanalyzing all files)...")
                args.reanalyze = True
                try:
                    _run_processing(args, base)
                except Exception as exc:
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="mAIpper v0.10 — Pentest scan analysis and Obsidian vault generator"
    )
        ...
        if misc_dir:
            ...
            if not misc_data["summary"]["total_files"]:
                ...
                misc_results = create_misc_vault(
                    vault_dir, misc_data,
                    analysis_by_file if not args.no_ollama else None,
                    None if args.no_ollama else args.model,
                )
