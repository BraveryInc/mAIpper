...
"""
mAIpper v0.11 - Pentest Tool Analysis & Obsidian Export Tool
...
_MISC_TOOL_HINTS: dict[str, str] = {
    ...
    "crackmapexec": "This is CrackMapExec/NetExec output. Focus on successful authentications, share access, and command execution results.",
...
def _reparse_and_analyze_scan(
    ...
    if label_lower == "nmap":
        ...
        raw = ollama_chat(args.ollama_url, args.model, p2_prompt, args.temperature)
        if not args.skip_validation:
...
def _process_eat_me_page(vault_dir: Path, args=None) -> dict | None:
    ...
    if not discovered_ips and not discovered_hostnames and not discovered_creds:
    ...
    # Analysis checkbox + original content
    scan_lines += [
        "",
        "- [ ] Analyze: Misc",
        "",
    ...
    # ── Save credentials to loot if found ──
    if discovered_creds:
        loot_dir = vault_dir / "Loot"
        loot_dir.mkdir(exist_ok=True)
        cred_lines = []
        for c in discovered_creds:
            cred_lines.append(f"{c['username']}:{c['password']}")
        loot_file = loot_dir / f"eat_me_{timestamp}.txt"
        # Don't write to Loot/ vault output — write to scans/loot/ source
        # so the loot pipeline picks it up on re-run
        pass  # Creds are captured in the scan note; loot pipeline handles source files
...
def _watch_loop(args, base: Path) -> None:
    ...
    print("      /deepdive   — full LLM analysis of all scans")
        ...
        while True:
            ...
            if not user_input:
                ...
                if pending_investigate or pending_analyze:
                    ...
                    print(f"  [*] Pending: {', '.join(parts)} — run /analyze to process")
                elif not any_change:
            ...
            if user_input.lower() in ("/quit", "/exit", "/q"):
                ...
                if args.no_ollama:
                    ...
                    print("  Running full analysis pipeline (all scans)...")
                    try:
                        args.reanalyze = True
                        _run_processing(args, base, skip_llm=False)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="mAIpper v0.11 — Pentest scan analysis and Obsidian vault generator"
    )
    ...
    if args.init:
        ...
        if not conf_path.exists():
            ...
            print(f"[+] Created default config: {conf_path.resolve()}")
        else:
            print(f"[*] Config already exists: {conf_path.resolve()}")
        return
